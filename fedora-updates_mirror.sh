...
# set VARS
    ...
    R_DIR=/limbus/centos/fedora/updates
    P_DATEFILE=FEDORA-UPDATES.LAST_UPDATE
    P_LOG=$P_DIR/FEDORA-UPDATES_LAST_MIRRORED.$DATE
    O_LOG=$P_DIR/FEDORA-UPDATES_LAST_MIRRORED.$YEST_DATE	#O is for OLD
    T_LOG=$P_DIR/FEDORA-UPDATES_MIRROR_IN_PROGRESS.$DATE
    E_LOG=$P_DIR/FEDORA-UPDATES_BROKE_ON.$DATE			#E is for ERROR
    RSYNC_COMMAND=/usr/local/share/rutgers-mirror-scripts/run-mirror-fedora-updates.sh
...
then
    ...
    chmod -R 755 $P_DIR
    chown -R root:root $P_DIR
OUTOFDATE=`find /limbus/centos/status -mtime -1 -name 'FEDORA-UPDATES_LAST_MIRROR*' | wc -l`
   if [ "$OUTOFDATE" -eq 0 ]
   then
   echo "Warning: FEDORA-UPDATES mirror out of date by 24 hours, please check logs at http://centos.rutgers.edu/mirror/status " | mail -s "FEDORA-UPDATES MIRROR 24 Hours out of date" $EMAIL
   fi
fi
